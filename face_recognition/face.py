...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def avatar_timer_func(self):
        ...
        if  not self.sign_show_flag:
            self.verticalLayoutWidget_2.hide()
            self.label_userinfo_all_bg.hide()
            self.sign_show_flag=True
        else :
            self.sign_show_flag = False
            if(int(time.time()) - self.avatar_time > self.avatar_between):
                self.verticalLayoutWidget_2.hide()
                self.label_userinfo_all_bg.hide()
            i = time.time()
            #person=self.getperson(self)
            ...
            if msg:
                ...
                if type(data) is dict:
                    ...
                    if person:
                        this_avatar = person["image"]
                        this_name=person["name"]
                        this_rank=person["position"]
                        this_department=person["department"]
                        self.label_2.setStyleSheet("border-image: url(" + this_avatar + ");")
                        # 设置名字
                        self.label_userinfo_all_name.setText('姓名: '+person["name"])
                        # 设置性别
                        #self.label_userinfo_all_sex.setText('性别：男')
                        # 设置年龄
                        #self.label_userinfo_all_age.setText('年龄：' + str(i))
                        # 设置职位
                        self.label_userinfo_all_rank.setText('职位：'+person["position"])
                        # 设置部门
                        self.label_userinfo_all_dept.setText('部门：'+person["department"])
                        # 设置签名
                        #self.label_userinfo_all_slogan.setText('签名：测试')
                        ...
                        self.this_time = time.strftime("%H:%M", time.localtime(time.time()))
                        self.this_date = time.strftime("%Y-%m-%d",time.localtime(time.time()))
